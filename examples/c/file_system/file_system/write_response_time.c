...
#include <signal.h>
#include "write_response_time.skel.h"	//包含了 BPF 字节码和相关的管理函数
...
static int handle_event(void *ctx, void *data,unsigned long data_sz)
{
	...
	printf("opcode:write\t pid:%d\t duration_time:%llums\t comm:%s\n", e->pid, e->duration_ns, e->comm);
...
int main(int argc, char **argv)
{
	...
	struct write_response_time_bpf *skel;
	int err;
	...
	*/
	...
	/* 打开BPF应用程序 */
	skel = write_response_time_bpf__open();
	if (!skel) {
	...
	/* 加载并验证BPF程序 */
	err = write_response_time_bpf__load(skel);
	if (err) {
	...
	/* 附加跟踪点处理程序 */
	err = write_response_time_bpf__attach(skel);
	if (err) {
...
cleanup:
	...
	write_response_time_bpf__destroy(skel);
