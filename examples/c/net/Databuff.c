...
static char netfilter_file_path[1024];
...
static const struct argp_option opts[] = {
    ...
    {"netfilter", 'F', 0, 0, "Show netfilter massage"},
    {"total", 'A', NULL, 0, "Show total massage"},
...
enum MonitorMode get_monitor_mode() {
    if(env.drop) {
        ...
        return MODE_DEFAULT;
    } else if(env.netfilter) {
        return MODE_NETFILTER_MSG;
    }
    else {
        return MODE_DEFAULT;
    }
...
static void open_log_files() {
    ...
    fclose(socket_file);
    FILE *netfilter_file = fopen(netfilter_file_path, "w+");
    if (netfilter_file == NULL) {
        fprintf(stderr, "Failed to open udp.log: (%s)\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
    fclose(netfilter_file);
...
void handle_drop(void* ctx, int cpu, void* data, __u32 data_sz) {
    ...
    if ((event -> daddr & 0x0000FFFF) == 0x0000007F ||
        (event -> saddr & 0x0000FFFF) == 0x0000007F)
        return ;
...
static int handle_dns(void *ctx, void *event, size_t size) {
    ...
    if ((pack_info -> daddr & 0x0000FFFF) == 0x0000007F ||
        (pack_info -> saddr & 0x0000FFFF) == 0x0000007F)
        return 0;
    char d_str[INET_ADDRSTRLEN];
    char s_str[INET_ADDRSTRLEN];
...
static int handle_transmit(void *ctx, void *event, size_t size) {
    ...
    if ((data -> daddr & 0x0000FFFF) == 0x0000007F ||
        (data -> saddr & 0x0000FFFF) == 0x0000007F)
        return 0;
...
static int handle_socket(void *ctx, void *event, size_t size) {
    ...
    fprintf(file,
            "%s, %s, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %f\n",
            inet_ntop(AF_INET, &saddr, s_str, sizeof(s_str)), 
            inet_ntop(AF_INET, &daddr, d_str, sizeof(d_str)), 
            ((data->sndbuf) / 1024),((data->sk_wmem_queued) / 1024),((data->sk_sndbuf) / 1024),
            data->qlen,((data->rcv_wup) / 1024),((data->rcv_nxt) / 1024),
            ((data->rcv_wnd) / 1024),data->receive_window,
            ((data->rmem_alloc) / 1024),((data->sk_rcvbuf) / 1024),data->tcp_data,
            data->tcp_ofo, ((data->SRTT) / 1000000.0)
            );
    fflush(file);
    fclose(file);
...
void handle_hands(void* ctx, int cpu, void* data, __u32 data_sz) {
    ...
    if ((e -> daddr_v4 & 0x0000FFFF) == 0x0000007F ||
        (e -> saddr_v4 & 0x0000FFFF) == 0x0000007F)
        return ;
...
static int print_netfilter(void *ctx, void *packet_info, size_t size) {
    FILE *file = fopen(netfilter_file_path, "a+"); // 追加
    if (file == NULL) {
        fprintf(stderr, "Failed to open udp.log: (%s)\n", strerror(errno));
        return 0;
    }
...
int main(int argc, char** argv) {
    ...
    strcat(netfilter_file_path, "data/netfilter.log");
    ...
    while (!exiting) {
        ...
        err = ring_buffer__poll(netfilter_rb, PERF_POLL_TIMEOUT_MS);
