...
int _tcp_sendmsg_locked(struct sock* sk) {
    ...
    // sk->sk_sndbuf: 当前连接发送缓冲区的最大限制。sndbuf不能大于sk_sndbuf
...
int _tcp_rcv_established(struct sock* sk) {
    ...
    // 零窗口判断 使用 receive_window < 0 判断零窗口
...
// 重排序 tcp_data_queue 调用 tcp_ofo_queue 进行重排序
// 思路：统计 tcp_data_queue 次数作为数据包的加入队列次数, 统计 tcp_ofo_queue 调用次数看作乱序处理次数
static __always_inline
